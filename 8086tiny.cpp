...
#ifdef _WIN32
#define KEYBOARD_DRIVER kbhit() && (mem[0x4A6] = getch(), pc_interrupt(7))c replace defines constants
#else
...
int main(int argc, char **argv)
{
   ...
   fprintf(fp,"DEBUG messages ON");
   //fprintf(fp, IO_PORT_COUNT);
   fclose(fp);
#endif // DEBUG
		...
		switch (xlat_opcode_id)
		{
			...
			OPCODE 48: // Emulator-specific 0F xx opcodes
				switch ((char)i_data0)
				{
					...
					OPCODE 2: // DISK_READ
					OPCODE_CHAIN 3: {
					if (~lseek(disk[regs8[REG_DL]], CAST(unsigned)regs16[REG_BP] << 9, 0))
					{
					   if ((char)i_data0 == 3) {
					    (int(*)())write(disk[regs8[REG_DL]] ,mem + SEGREG(REG_ES, REG_BX,), regs16[REG_AX]); } else {
